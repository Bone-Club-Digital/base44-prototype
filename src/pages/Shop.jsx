...
import RealMoneyProductCard from '../components/shop/RealMoneyProductCard';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setProducts(productList.filter(p => p.real_price && p.real_price > 0));
    ...
    <div className="p-4 sm:p-8" style={{ backgroundColor: '#e5e4cd' }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl sm:text-5xl text-[#5a3217]">Bone Club Shop</h1>
          <p className="main-text mt-2">Purchase premium backgammon equipment with real money.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map(product => (
              <RealMoneyProductCard key={product.id} product={product} />
            ))}
          </div>
        )}
